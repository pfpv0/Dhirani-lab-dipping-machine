...
class PointsTable:
    ...
    def get_array(self):
        ...
        for key in tempiid:
            ...
            p_temp = str(round(40 * float(p_temp)))
...
class SerialWidget:
    ...
    def __init__(self, master):
        ...
        self.ports_label = tk.Label(text='Available ports')

        # List box of all the serial connections
        self.ports = list_ports.comports()
        self.devices = [p.device for p in self.ports]
        self.deviceList = tk.Listbox(master, height=3)
        ...
        self.monitor = tk.Text(master, height=12, width=40)
        ...
        # Layout
        #self.ports_label.grid(row=0, column=0)
        self.deviceList.grid(row=1, column=0)
        self.connectButton.grid(row=2, column=0)

        self.monitor.grid(row=0, column=1, rowspan=4)
        self.serialEntry.grid(row=3, column=0)
...
serial_frame.grid(row=1, column=0, sticky=tk.NE)

print_button = tk.Button(root, text='SUBMIT', command=submit_to_terminal)
print_button.grid(row=0, column=1, columnspan=2)

print(serial_frame.winfo_screenwidth())

print(serial_frame.winfo_width())


def on_closing():
    mySerial.arduino.close()
    root.destroy()
