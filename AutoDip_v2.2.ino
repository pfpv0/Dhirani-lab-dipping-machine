...
//maximum x position
int xposmax = 200;
//leave as true, starts in joystick mode
bool joymode = true;


//Variables for running the dipping program (change these as needed)
...
void loop() {
  ...
  while (joymode == false && cyclecount < nlayers) {
    ...
    //goes back into joymode if the cycle ends 
    if (cyclecount == nlayers) {
      joymode = true;
    }
  }
}
...
void dipcycle() {
  for (int j = 0; j < ndips; j++) {
    ...
    xMoveSteps(positions[(j + 1) % ndips] - positions[j % ndips], 1000);
  }
}
